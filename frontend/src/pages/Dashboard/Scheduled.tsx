  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        <div className="mx-auto w-12 h-12 rounded-2xl bg-rose-500/10 text-rose-400 border border-rose-500/20 flex items-center justify-center mb-3">
  ...
  if (emails.length === 0) {
    return (
      <EmptyState
        ...
        description="There are currently no scheduled emails waiting in the BullMQ queue. Compose a new campaign to schedule emails."
      />
    );
  }
              ...
              <tr key={email.id} className="hover:bg-slate-800/40 transition-colors group">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-slate-800/90 text-blue-400 border border-slate-700/80 flex items-center justify-center flex-shrink-0 group-hover:border-blue-500/40 transition-colors">
          ...
          <div key={email.id} className="p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 truncate">
                ...
                <span className="text-xs font-semibold text-white truncate">{email.recipient}</span>
              </div>
              <Badge status={email.status} />
            </div>
