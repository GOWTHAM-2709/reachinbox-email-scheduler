...
export const DashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-mesh-dark text-slate-100 flex flex-col font-sans selection:bg-blue-500 selection:text-white">
      {/* Top Glass Navigation Bar */}
      <header className="sticky top-0 z-40 glass-panel border-b border-slate-800/80 shadow-lg shadow-black/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            {/* Brand Logo & Landing Link */}
            <div className="flex items-center gap-3">
              <Link to="/" className="flex items-center gap-2.5 group">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center text-white shadow-md shadow-blue-500/30 group-hover:scale-105 transition-transform">
            ...
            <nav className="hidden md:flex items-center gap-1.5 p-1 bg-slate-900/80 rounded-2xl border border-slate-800">
            ...
            <div className="hidden sm:flex items-center gap-3">
              {/* Slack Status / Action */}
              {user.slackConnected ? (
                <div className="flex items-center gap-2 p-1 pl-2.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                  ...
                  <button
                    ...
                    className="text-[11px] font-medium text-slate-400 hover:text-rose-400 px-2 py-0.5 rounded-full hover:bg-slate-800 transition-colors"
                  >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-slate-200 bg-slate-800/90 border border-slate-700/80 rounded-xl hover:bg-slate-700 hover:border-slate-600 transition-all shadow-sm active:scale-95"
                >
                  <img
              ...
              <div className="flex items-center gap-2.5 pl-3 border-l border-slate-800">
                {user.avatar ? (
                  ...
                  <div className="h-8 w-8 rounded-full bg-blue-600/20 border border-blue-500/30 text-blue-400 flex items-center justify-center font-bold text-xs">
              ...
              <button
                onClick={logout}
                title="Log out"
                className="text-slate-400 hover:text-rose-400 p-2 rounded-xl hover:bg-slate-800/80 transition-colors"
              >
        ...
        {isMobileMenuOpen && (
          <div className="sm:hidden p-4 border-t border-slate-800 bg-slate-900/95 backdrop-blur-xl space-y-3 animate-in">
            ...
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
              {user.slackConnected ? (
                ...
                <button
                  onClick={handleSlackConnect}
                  className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-slate-800 text-xs font-medium text-slate-200"
                >
            ...
            <button
              onClick={logout}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-rose-500/10 text-rose-400 border border-rose-500/20 text-xs font-medium"
            >
